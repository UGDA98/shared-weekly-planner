...
**Month view + fri/sat split + state chunking are committed locally but NOT deployed** (2026-09-09). Nothing has shipped to users yet. Next steps, in order: measure real board sizes (Watch Out #15), deploy the backend per "Deploy ordering" above, verify on a scratch board, then push the frontend.

Not yet done from the plan: the old-client round-trip test has been verified only against a replica of the old `loadWeek` logic in a unit test, not against the actual previous `index.html` served side by side. Worth doing that once before pushing, since it is the highest-risk interaction in the change.
...
15. **Board size has never been measured on the real boards.** Do a read-only `?action=getBoard` and record `JSON.stringify(json.data).length` per board; that number decides how urgent #13 and #14 are.
